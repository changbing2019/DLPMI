"""
edwards_dlpmi_estimate.py  (with uncertainty analysis)
======================================================
...
-----------------------
  ...
  edwards_dlpmi_physics.py     ← tracer input functions, DM/BMM forward models,
                                 PINN classes, loss, training, uncertainty fns
  edwards_dlpmi_estimate.py    ← THIS FILE: load → fit → uncertainty → plot → CSV
...
Usage
-----
  python edwards_dlpmi_estimate.py                   # all 65 samples, all methods
  python edwards_dlpmi_estimate.py --ids EDTRPAS1-29 EDTRPAS1-36
  python edwards_dlpmi_estimate.py --starts 3 --adam 5000 --mc 100
  python edwards_dlpmi_estimate.py --no-profile      # skip slow profile CI
  python edwards_dlpmi_estimate.py --no-mc           # skip Monte Carlo

Outputs (./outputs/edwards_dlpmi/ by default)
--------------------------------------------
  {SampleID}_dlpmi.png          4-panel figure + uncertainty panel
...
def train_dlpmi(model, loss_fn, n_adam=5000, lr_adam=3e-3, lr_lbfgs=0.02):
...
_OUT_DIR        = os.path.join(_HERE, "results", "edwards_dlpmi")
...
def _compute_hessian(s, model, tns_a, obs_a, scls_a, he4r, dic_c1, dic_c2, chi2_val):
    ...
    if lpm == "DM":
        ...
        bounds_lo   = [float(s["dlpmi_bounds"]["tau1_lo"]),
                       float(s["dlpmi_bounds"]["pd1_lo"])]
        bounds_hi   = [float(s["dlpmi_bounds"]["tau1_hi"]),
                       float(s["dlpmi_bounds"]["pd1_hi"])]
        ...
        tau1_hi_b  = min(float(s["dlpmi_bounds"]["tau1_hi"]), t2v*0.9)
        bounds_lo  = [float(s["dlpmi_bounds"]["tau1_lo"]), 0.005]
...
def _compute_mc(s, model, tns, tns_a, obs_all, obs_errs_all,
    ...
    if lpm == "DM":
        t2v = None; f1v = None
        tau1_lo = float(s["dlpmi_bounds"]["tau1_lo"])
        tau1_hi = float(s["dlpmi_bounds"]["tau1_hi"])
        pd1_lo  = float(s["dlpmi_bounds"]["pd1_lo"])
        pd1_hi  = float(s["dlpmi_bounds"]["pd1_hi"])
    else:
        pars     = model.get_params()
        f1v      = pars[2].item(); t2v = pars[3].item(); pd2v = pars[4].item()
        tau1_lo  = float(s["dlpmi_bounds"]["tau1_lo"])
        tau1_hi  = min(float(s["dlpmi_bounds"]["tau1_hi"]), t2v*0.9)
        pd1_lo   = float(s["dlpmi_bounds"]["pd1_lo"])
        pd1_hi   = float(s["dlpmi_bounds"]["pd1_hi"])
    ...
    for _ in range(n_mc):
        ...
        try:
            if lpm == "DM":
                ...
                train_dlpmi(m, lfn, min(n_adam//5, 800))    # warm-start: fewer epochs
                tv, pv = m.get_params()
                ...
                train_dlpmi(m, lfn, min(n_adam//5, 800))
                t1,_,f1,t2,_ = m.get_params()
...
def plot_sample(res, out_dir):
    ...
    out_path = os.path.join(out_dir, f"{sid.replace('/','_')}_dlpmi.png")
...
def plot_summary(all_results, out_dir):
    ...
    for ax, metric, title, xlabel, log_x in panels:
        ...
        dlpmi_v = [r[metric] or 0. for r in all_results]
        ...
        y=np.arange(n)
        ax.barh(y-0.20, dlpmi_v, 0.38, color=pc, alpha=0.87, label="PINN")
            ...
            for i,r in enumerate(all_results):
                ...
                if "tau1" in pn:
                    ...
                    if idx < len(sg) and sg[idx] is not None:
                        ax.errorbar(dlpmi_v[i], y[i]-0.20,
                                    xerr=[[min(sg[idx],dlpmi_v[i]*0.99)],[sg[idx]]],
                ...
                if mc.get("p16") and mc.get("p84"):
                    ax.errorbar(dlpmi_v[i], y[i]-0.20,
                                xerr=[[dlpmi_v[i]-mc["p16"]],[mc["p84"]-dlpmi_v[i]]],
